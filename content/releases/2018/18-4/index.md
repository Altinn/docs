...
Varslingsadresse for virksomheten vil benyttes av Altinn til usending av varsel via Altinn. Varslingsadress for allerede registrere virksomheter kan oppdateres via Profl i Altinn. 
{{<figure src="1.PNG?width=600" title="Oppdater varslingsinformasjon i profil" >}}

Ettersom alle virksomheter **er pålagt å ha varslingsadresse** så vil de mangler denne bli avkrevd en slik adresse når personer med tilgangsstyring går inn på virksomheten første gang etter 18.4. 
{{<figure src="2.PNG?width=600" title="Registrer varslingsinformarsjon" >}}

Varslingsadresse registrert eller endret i Altinn vil umiddelbart sendes til enhetsregisteret for registrering. Etter en kort tid så vil Altinn få tilbakemelding fra Enhetsregisterte. Merk at det i mellomtiden ikke vil være mulig å korrigere registrert varslingsadresse. 
{{<figure src="3.PNG?width=600" title="Venter paa tilbakemelding" >}}
.
## Portal
...
På skjema i innboks vises tidspunkt for når skjema er opprettet. Tidligere ble denne feilaktig benevnt som "mottatt". Den er nå korrigert til "opprettet"
### Forenklet del og gi tilgang til meldingstjeneste
Prosess for å dele og gi tilgang til en melding du har mottatt er forenklet
### Forbedringer i  Søk på tvers
Det er nå enklere å slette lagrede søk og vi har gjort det lettere å komme tilbake til søket etter å ha åpnet kvittering på arkivert skjema. I tillegg er det nå lettere å se hvilke aktører du har valgt å søke på. 
{{<figure src="4.PNG?width=600" title="Valgte aktorer" >}}
### Bedre tilbakemelding til bruker som signerer/sender inn på vegne av andre
Det er mulig å gi en bruker tilgang til bare et spesielt eksemplar av et skjema. Når bruker signere og sender inn dette på vegne av noen andre så vil ikke bruker lenger ha tilgang til å se på skjema som er sendt inn. Det samme vil være tilfellet hvis man som siste person sender inn et skjema som har paralell signering. 
...
### Vaske XML for InfoPath elementer i REST
Som tjenesteeier ønsker jeg at InfoPath elementer ikke skal vises i REST

Før inneholdt XML lagret i Altinn InfoPath elementer. Nå blir disse elementene fjernet fra XMLen når det hentes ut.
### Uthenting av elementer fra søppelbøtta via REST
...
Det er lagt til mulighet for å hente ut FormData for elementer i innboksen via WebSerivce. Kall til `ServiceEngineExternal/ReporteeElementListExternal/GetFormSetData` henter ut alle XML-skjemaer for et element. Skjemaene blir vasket for interne felter og namespaces på lik linje med REST ({{<tfs 17260 story>}}).
...
Som bruker av REST API får man nå ut mer informasjon om Formtasks. Det er lagt til en liste med prosess-steg (ProcessSteps), som inneholder navn og sikkerhetsnivå som kreves for å fullføre hvert steg.
Det er lagt til 2 felter som indikerer om endebrukersystemer kan brukes for Formtasken (EUSEnabled/EnterpriseUserEnabled).  
Se REST APIets hjelpesider for mer informasjon: `/api/Help/`
...
Da forhindrer vi at to forskjellige cache keys kan bli brukt ved et uhell.  
...
Det er laget en ny resurs `/api/metadata/codelists` hvor en ekstern person/utvikler kan få listet opp alle kodelistene i Altinn.
Listen filtreres i utgangspunktet automatisk på norsk, men bruker kan endre denne filtreringen ved å sette språk selv.  
Se REST APIets hjelpesider for mer informasjon: `/api/Help/`

MetadataController er blitt utvidet med 2 metoder. Den ene returnerer en komplett oversikt over kodelister på et gitt språk. Den andre
metoden returnerer en konkret kodeliste basert på navn, versjon og språk. Navn er eneste obligatoriske felt. Standardverdi for versjon er `0`,
noe som gir siste versjon av en kodeliste. Standardverdi for språk er norsk.
### Endre avsender på skjema man instansierer via prefill-webservices
Det er lagt til 2 felt som skal inneholde organisasjons nummer til den virksomheten man sender på vegne av.  
Se REST APIets hjelpesider for mer informasjon: `/api/Help/`
...
Det var tidligere ikke mulig å slette elementer for organisasjoner gjennom REST APIet. Dette er endret slik at det foretas en soft-delete for elementer som eies av organisasjoner i stede.

### Organisasjonsnummer for signerende bruker i feltet SignedByUserSsn for webservice-kall til GetArchivedFormTask
